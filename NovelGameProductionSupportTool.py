...
from typing import List, Dict, Type, Optional, Any, Tuple, Callable
import math
...
class ShortcutEntry(ttk.Entry):
    ...
    def _on_key_press(self, event):
        ...
        if key_name in ["CONTROL_L", "CONTROL_R", "SHIFT_L", "SHIFT_R", "ALT_L", "ALT_R", "CAPS_LOCK"]:
            return "break"
            
        key_map = {"PLUS": "plus", "MINUS": "minus", "EQUAL": "plus"}
        key_name = key_map.get(key_name, key_name)
        
        shortcut_parts = sorted(modifiers) + [key_name]
...
class TextWithLineNumbers(tk.Frame):
    def __init__(self, master, **kwargs):
        ...
        self.font = kwargs.get("font") or ("TkDefaultFont", 10)
        ...
        kwargs['yscrollcommand'] = self._on_text_scroll
        self.text = tk.Text(self, relief=tk.FLAT, **kwargs)
        self.text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        self.scrollbar.config(command=self.text.yview)
        self.text.bind("<<Modified>>", self._schedule_update)
        self.text.bind("<Configure>", self._schedule_update)
    ...
    def __getattr__(self, name):
        try:
            ...
            raise AttributeError(f"'{type(self).__name__}' object (or its internal Text widget) has no attribute '{name}'")
...
class PluginManager:
    def __init__(self, app: 'NovelGameEditor'):
        ...
        self._setup_plugin_path()

    def _setup_plugin_path(self):
        """プラグインパスの設定"""
...
class ConfigManager:
    ...
    def _load_config(self) -> None:
        ...
        default_shortcuts = {
            ...
            'add_scene': 'Control-A',
            'add_branch': 'Control-B',
            'zoom_in': 'Control-plus',
            'zoom_out': 'Control-minus',
            'reset_view': 'Control-0'
        }
    ...
    def get_shortcut_display(self, action: str) -> str:
        """表示用のショートカット文字列を取得"""
        return self.get_shortcut(action).replace('-', '+')
    ...
    def add_recent_file(self, path: Path):
        ...
        # 既存のオプションをクリア
        for key in self.config.options('RECENT_FILES'):
            self.config.remove_option('RECENT_FILES', key)
            
        # 新しいリストを保存 (最大5件)
        for i, p in enumerate(recent_files[:5]):
            self.config.set('RECENT_FILES', f'file_{i}', str(p))
...
class SettingsDialog(tk.Toplevel):
    ...
    def _create_widgets(self) -> None:
        ...
        for i, (action_key, action_label) in enumerate(shortcut_actions.items(), 1):
            ...
            entry = ShortcutEntry(main_frame, width=30)
...
class SceneSelectionDialog(tk.Toplevel):
    ...
    def _create_widgets(self):
        ...
        self.listbox.bind("<Return>", self._on_ok)
...
class BranchDialog(tk.Toplevel):
    def __init__(self, parent, title, all_scenes, source_scene, app: 'NovelGameEditor', 
        ...
        self._create_widgets(initial_text, initial_condition)
        self.resizable(False, False)
        self.wait_window(self)
    ...
    def _create_widgets(self, initial_text, initial_condition):
        ...
        ttk.Label(frame, text="選択肢テキスト:").grid(
        ...
        target_entry = ttk.Entry(frame, textvariable=self.target_scene_var, state="readonly")
        target_entry.grid(row=1, column=1, sticky="ew", padx=5, pady=5)
        
        ttk.Button(frame, text="選択...", command=self._select_scene).grid(
            row=1, column=2, padx=5
        )
        
        ttk.Label(frame, text="条件 (任意):").grid(
    ...
    def _select_scene(self):
        ...
        if not available_scenes:
            if messagebox.askyesno(
                ...
                if new_scene:
                    self.target_id = new_scene.id
                    self.target_scene_var.set(new_scene.name)
            return
        ...
        if dialog.result:
            ...
            if selected_scene:
                self.target_scene_var.set(selected_scene.name)
...
class PluginManagementDialog(tk.Toplevel):
    ...
    def _create_widgets(self):
        ...
        ttk.Label(main_frame, text="インストール済みプラグイン:", font="-weight bold").pack(
            anchor="w", pady=(0, 10)
        )
        ...
        info_label = ttk.Label(
            main_frame,
            text="変更を反映するには、アプリケーションの再起動が必要です。",
            foreground="orange"
        )
        ...
        ttk.Button(button_frame, text="保存して閉じる", command=self._save_and_close).pack(
            side=tk.LEFT, padx=5
        )
...
class NovelGameEditor:
    def __init__(self, root: tk.Tk):
        ...
        self.plugin_menu: Optional[tk.Menu] = None
        self.recent_files_menu: Optional[tk.Menu] = None
        self.bound_shortcuts: List[str] = []  # バインドされたショートカットを追跡するリスト
    ...
    def add_plugin_menu_command(self, label: str, command: callable):
        ...
        self.menubar.entryconfig("プラグイン", state="normal")
    ...
    def remove_plugin_menu_command(self, label: str):
        ...
        try:
            ...
            if self.plugin_menu.index("end") is None:
                self.menubar.entryconfig("プラグイン", state="disabled")
        except tk.TclError:
            pass
    ...
    def _create_editor_widgets(self, parent_frame):
        ...
        ttk.Label(scene_info_frame, text="シーン名:").grid(
        ...
        self.editor_plugin_frame.grid(
            row=1, column=0, columnspan=2, sticky="ew", pady=(5, 0), padx=5
        )
        
        ttk.Label(scene_info_frame, text="内容:").grid(
            row=2, column=0, sticky="nw", pady=(5,2)
        )
        
        self.scene_content_text = TextWithLineNumbers(
            scene_info_frame,
            wrap=tk.WORD,
            height=10
        )
        ...
        self.add_branch_btn = ttk.Button(
            branch_btn_frame,
            text="追加",
        ...
        self.edit_branch_btn = ttk.Button(
            branch_btn_frame,
            text="編集",
            command=self.edit_branch,
            width=8
        )
        ...
        # ツールチップ
        Tooltip(self.add_branch_btn, 
               f"分岐を追加 ({self.config_manager.get_shortcut_display('add_branch')})")
    ...
    def _on_canvas_drag(self, event):
        ...
        # 一定距離をドラッグした場合にのみ "moved" フラグを立てる
        if not self.drag_state.get("moved"):
            dist_sq = (event.x - self.drag_state["start_x_screen"])**2 + \
                     (event.y - self.drag_state["start_y_screen"])**2
            if dist_sq >= DRAG_THRESHOLD_SQUARED:
    ...
    def _on_canvas_right_click(self, event):
        ...
        context_menu.add_command(
            label="ここにシーンを追加",
            command=lambda: self.add_scene(at_canvas_pos=(cx, cy)))
        context_menu.add_command(
            label="ビューをリセット",
    ...
    def _draw_branches(self):
        ...
        for scene in self.scenes:
            for branch in scene.branches:
                ...
                start_offset_x = start_x + (dx / dist) * DEFAULT_NODE_RADIUS
                start_offset_y = start_y + (dy / dist) * DEFAULT_NODE_RADIUS
                end_offset_x = end_x - (dx / dist) * DEFAULT_NODE_RADIUS
                end_offset_y = end_y - (dy / dist) * DEFAULT_NODE_RADIUS
                ...
                self.canvas.create_line(
                    ...
                    s_end_x, s_end_y,
                    fill="#999999",
                    width=1.5 * self.scale,
                    arrow=tk.LAST
                )
                
                mid_x, mid_y = (s_start_x + s_end_x) / 2, (s_start_y + s_end_y) / 2
                font_size = max(7, int(9 * self.scale))
                cond_font_size = max(6, int(8 * self.scale))
                font = ("Arial", font_size)
                cond_font = ("Arial", cond_font_size, "italic")
                
                self.canvas.create_text(
                    mid_x, mid_y - (6 * self.scale),
                    text=branch["text"],
                    fill="#CCCCCC",
                    font=font
                )
                ...
                if branch["condition"]:
                    self.canvas.create_text(
                        mid_x, mid_y + (6 * self.scale),
                        text=f"[{branch['condition']}]",
                        fill="#AAAAAA",
                        font=cond_font
                    )

    def _check_dirty_and_proceed(self) -> bool:
        """変更があるか確認し、処理を続行するかどうかを返す"""
    ...
    def new_project(self, event=None, startup=False):
        """新規プロジェクトを作成"""
        if not startup and not self._check_dirty_and_proceed():
            return
    ...
    def open_project(self, event=None, path_to_open: Optional[Path] = None):
        """プロジェクトを開く"""
        if not self._check_dirty_and_proceed():
            return
            
        path_str = str(path_to_open) if path_to_open else filedialog.askopenfilename(
            filetypes=[("ノベルプロジェクト", "*.ngp")]
        )
    ...
    def _update_recent_files_menu(self):
        ...
        for i, path in enumerate(recent_files):
            path_text = str(path)
            if len(path_text) > 60:
                path_text = path_text[:25] + "..." + path_text[-30:]
                
            self.recent_files_menu.add_command(
                label=f"{i+1}. {path_text}",
                command=lambda p=path: self.open_project(path_to_open=p)
            )
    ...
    def _update_menu_accelerators(self):
        ...
        self.file_menu.entryconfig(
            "名前を付けて保存...",
            accelerator=self.config_manager.get_shortcut_display('save_project_as')
        )
    ...
    def delete_scene(self, event=None):
        ...
        if not messagebox.askyesno(
            "確認",
            f"シーン '{self.selected_scene.name}' を削除しますか？\nこのシーンへの分岐も全て削除されます。"
        ):
            return
    ...
    def delete_branch(self, event=None):
        """分岐を削除"""
        if not self.selected_scene or not self.branch_tree.selection():
            return
        ...
        indices_to_delete = sorted(
            [self.branch_tree.index(iid) for iid in self.branch_tree.selection()],
            reverse=True
        )
